...
function startGame() {
  //Choose board diffuculty
  let board;
  if (id("diff-1").checked) board = easy[0];
  else if (id("diff-2").checked) board = medium[0];
  else board = hard[0];
  //Set lives and enable  selecting number ant tiles
  lives = 3;
  disableSelect = false;
...
function startTimer() {
  // Sets time remainingon input
  if (id("time-1").checked) timeRemaining = 180;
  else if (id("time-2").checked) timeRemaining = 360;
...
// Convert secont into mm:ss formet
  ...
  for (let i = 0; i < 81; i++) {
    ...
    if (board.charAt(i) != "-") {
      ...
      tile.addEventListener("click", function () {
        ...
        if (!disableSelect) {
          // if th tile is already selected
...
function updateMove() {
  ...
  if (selectedTile && selectedNum) {
    ...
    if (checkCorrect(selectedTile)) {
      ...
      // If the number doesn not match the solution key
    } else {
      //Disable selecting new number for one secend
      disableSelect = true;
...
function checkCorrect(tile) {
  ...
  //If tiles umber is equal to solution number
